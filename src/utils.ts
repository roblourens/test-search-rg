...
export function createTextSearchResult(uri: vscode.Uri, fullText: string, range: vscode.Range, previewOptions?: vscode.TextSearchPreviewOptions): vscode.TextSearchResult {
	let preview: vscode.TextSearchMatchPreview;
	if (previewOptions) {
		...
		preview = {
			...
			matches: new vscode.Range(0, range.start.character - previewStart, 0, endOfMatchRangeInPreview)
		};
	} else {
		preview = {
			text: fullText,
			matches: new vscode.Range(0, range.start.character, 0, range.end.character)
		};
	}
